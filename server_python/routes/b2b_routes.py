...
def discover_orgs():
    ...
    (or whose connection was disconnected/blocked). Both org types can discover
    each other — no type restriction applied by default.
    ...
    # Only restrict by type if caller explicitly passes one
    if type_f in ("client", "provider"):
        org_filter["type"] = type_f
