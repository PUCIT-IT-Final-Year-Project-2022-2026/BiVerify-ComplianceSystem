...
function getComplianceStatus(days) {
  if (days < 0) return { pct: 0, cls: "d" };
  if (days <= 7) return { pct: 30, cls: "d" };
  if (days <= 30) return { pct: 60, cls: "w" };
  return { pct: 100, cls: "ok" };
}
...
export default function PartnerProfile({ connectionId, onBack }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Now use the actual partner org id for docs
      ...
      if (partnerId) {
        try {
          ...
          setDocs([]); // docs failing shouldn't break the whole page
        }
      }
    } catch (err) {
    ...
    return (
      <>
        ...
        <div className="page" style={{textAlign:"center",paddingTop:60}}>
          <div style={{fontSize:16,fontWeight:600,color:C.darkText,marginBottom:10}}>Partner not found</div>
          <button onClick={onBack} style={{fontSize:13,color:C.primary,background:"none",border:"none",cursor:"pointer",textDecoration:"underline"}}>← Back</button>
        </div>
      </>
    );
  }
  ...
  const compScores = [
    { label: "Compliance Coverage", pct: Math.floor(Math.random() * 40 + 60), cls: "ok" },
    { label: "Document Validity", pct: Math.floor(Math.random() * 30 + 70), cls: "ok" },
    { label: "License Status", pct: Math.floor(Math.random() * 40 + 50), cls: "w" },
  ];
    ...
    <>
      ...
      <nav className="topnav">
        ...
        <div className="nav-right">
          ...
          <button className="notif-btn"><Ico n="bell" s={15} c="rgba(255,255,255,0.85)"/></button>
        </div>
      </nav>
        ...
        <div className="body-grid fu fu2">
          <div className="body-left">
            ...
            <div className="details-grid">
              <div className="field">
                ...
                <div className="f-val f-mono">{p.id || connection.partner.id}</div>
              </div>
              <div className="field">
              ...
              <div className="field">
                ...
                <div className="f-val" style={{ color: C.primary, fontSize:12.5 }}>{p.adminEmail}</div>
              </div>
              <div className="field">
          ...
          <div className="body-right">
            <div>
              ...
              {docs.length === 0 ? (
                <div style={{fontSize:12,color:C.muted,textAlign:"center",padding:"20px 0"}}>No documents on file</div>
  ...
  {p.type === "provider" && (
    <div className="chip" onClick={() => showToast("Service request feature coming soon")}>
  ...
  >
    ...
    <span className="chip-lbl">Contact</span>
  </div>
        ...
        <div className="modal-ov" onClick={() => setShowModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-s">
              This will remove <strong>{p.name}</strong> from your B2B network. Active service bookings won't be affected, but future requests will be unavailable.
            </div>
            <div className="modal-btns">
              <button className="m-cancel" onClick={() => setShowModal(false)} disabled={disconnecting}>Cancel</button>
