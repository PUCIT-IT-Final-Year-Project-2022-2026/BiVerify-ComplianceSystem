...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .stat-card { background: ${C.card}; border: 1px solid ${C.border}; border-radius: 12px; padding: 14px 18px; display: flex; align-items: center; gap: 14px; transition: transform 0.15s; cursor: pointer; }
  ...
  .partner-row { display: flex; align-items: center; gap: 14px; padding: 13px 18px; border-bottom: 1px solid ${C.border}; transition: background 0.12s; cursor: pointer; }
  ...
  .recv-head { padding: 12px 16px; border-bottom: 1px solid ${C.border}; }
  .recv-head-t { display: flex; align-items: center; gap: 7px; font-size: 13px; font-weight: 600; color: ${C.darkText}; }
  .recv-count { background: ${C.primary}; color: #fff; border-radius: 20px; padding: 1px 7px; font-size: 11px; font-weight: 600; }
  .recv-row { display: flex; align-items: flex-start; gap: 10px; padding: 11px 14px; border-bottom: 1px solid ${C.border}; cursor: pointer; transition: background 0.12s; }
  .recv-row:last-child { border-bottom: none; }
  .recv-row:hover { background: ${C.bgLight}; }
  .recv-av { width: 34px; height: 34px; border-radius: 9px; background: ${C.bgIcon}; border: 1px solid ${C.borderMed}; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: ${C.dark}; flex-shrink: 0; font-family: 'DM Mono', monospace; }
  .recv-name { font-size: 13px; font-weight: 600; color: ${C.darkText}; }
  .recv-ind { font-size: 11px; color: ${C.muted}; }
  .recv-msg { font-size: 11px; color: ${C.muted}; margin-top: 4px; font-style: italic; }
  .recv-actions { display: flex; flex-direction: column; align-items: flex-end; gap: 5px; flex-shrink: 0; }
  .recv-time { font-size: 10.5px; color: ${C.muted}; white-space: nowrap; }
  .btn-accept { background: ${C.primary}; color: #fff; border: none; border-radius: 6px; padding: 4px 10px; font-size: 11.5px; font-weight: 600; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: background 0.13s; }
  .btn-accept:hover { background: ${C.dark}; }
  .btn-decline { background: transparent; color: ${C.muted}; border: 1px solid ${C.border}; border-radius: 6px; padding: 4px 8px; font-size: 12px; cursor: pointer; font-family: 'DM Sans', sans-serif; }
  ...
  .pend-head { padding: 12px 16px; border-bottom: 1px solid ${C.border}; }
  .pend-head-t { display: flex; align-items: center; gap: 7px; font-size: 13px; font-weight: 600; color: ${C.darkText}; }
  .pend-count { background: ${C.warning}; color: #fff; border-radius: 20px; padding: 1px 7px; font-size: 11px; font-weight: 600; }
  .pend-row { display: flex; align-items: center; gap: 10px; padding: 11px 14px; border-bottom: 1px solid ${C.border}; cursor: pointer; transition: background 0.12s; }
  .pend-row:last-child { border-bottom: none; }
  .pend-row:hover { background: ${C.bgLight}; }
  .pend-av { width: 34px; height: 34px; border-radius: 9px; background: ${C.bgIcon}; border: 1px solid ${C.borderMed}; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: ${C.dark}; flex-shrink: 0; font-family: 'DM Mono', monospace; }
  .pend-name { font-size: 13px; font-weight: 600; color: ${C.darkText}; }
  .pend-ind { font-size: 11px; color: ${C.muted}; }
  .pend-actions { display: flex; flex-direction: column; align-items: flex-end; gap: 5px; margin-left: auto; flex-shrink: 0; }
  .pend-sent { font-size: 10.5px; color: ${C.muted}; white-space: nowrap; }
  .btn-cancel { background: transparent; color: ${C.muted}; border: 1px solid ${C.border}; border-radius: 6px; padding: 4px 8px; font-size: 12px; cursor: pointer; font-family: 'DM Sans', sans-serif; }
...
export default function B2BNetwork() {
  ...
  const handleAccept = async (connId, name) => {
    try {
      ...
      showToast(`${name} is now connected!`);
      await loadData();
    } catch (err) {
      showToast(apiErrorMessage(err), "err");
    }
  ...
  const handleRemove = async (connId, name) => {
    try {
  ...
  const pending   = connections.filter(c => c.status === "pending" && c.direction === "outgoing");
  const received  = connections.filter(c => c.status === "pending" && c.direction === "incoming");
        ...
        <div className="split fu fu2">
          ...
          <div className="panel">
            <div className="panel-head">
              ...
              <div className="ph-badge">{filtered.length} active</div>
            </div>
            ...
            <div className="partner-list">
              {listLoading ? (
                ...
                <div className="partner-row" key={c.id} onClick={() => setSelectedPartner(c.id)}>
                  ...
                  <span className={`badge ${c.partner?.type === "provider" ? "b-provider" : "b-client"}`}>
                    <span className="bdot bdot-g"/>{c.partner?.type}
                  </span>
                  <div className="p-since">Since {fmtSince(c.connectedAt)}</div>
                </div>
              ))}
            </div>
          </div>
